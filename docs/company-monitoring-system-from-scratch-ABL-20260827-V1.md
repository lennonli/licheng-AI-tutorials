...
> **版本**：V1（20260827）｜**适用读者**：接手维护或在新环境复刻本系统的 AI Agent（ZCode 及同构 CLI 环境）
...
2. **小红书渠道口径**：小红书无合规可用的自动化通道（无登录态抓不到，带 cookie 属平台协议禁止项且相关开源工具多停更/禁商用）。两个选项——周度人工抽查（推荐，每周五日报附提醒清单）/付费商业 API；
...
| C 档 社媒 | 微博、微信公众号搜索、百度资讯 | 苗头感知，噪音最大，只做初筛 | playwright＋系统 Chrome；全部候选交研判环节定性 |
...
└── tools/
    ...
    ├── fetch_social.py           # 社媒三通道（微博m.weibo.cn API＋搜狗微信＋百度资讯）
...
### 8.3 官方公告降级通道 `fetch_official.py`
